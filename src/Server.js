...
(async () => {
  ...
  await client.query(`
    ...
    CREATE TABLE IF NOT EXISTS sessions (
      ...
      seeker_id UUID REFERENCES users(anonymous_id),
      helper_id UUID REFERENCES users(anonymous_id),
      started_at TIMESTAMPTZ DEFAULT NOW(),
      ended_at TIMESTAMPTZ,
      ended_early BOOLEAN DEFAULT FALSE,
      seeker_rating INT CHECK (seeker_rating BETWEEN 1 AND 5),
    ...
    CREATE TABLE IF NOT EXISTS achievements (
      ...
      UNIQUE(anonymous_id, achievement_type)
    );
...
async function checkAchievements(anonymous_id) {
  ...
  if (heat.rows[0]?.streak >= 7 && !existingTypes.includes('7_day_streak')) {
    newAchievements.push('7_day_streak');
  }
  if (heat.rows[0]?.streak >= 30 && !existingTypes.includes('30_day_streak')) {
    newAchievements.push('30_day_streak');
  }
  if (heat.rows[0]?.total_helped >= 10 && !existingTypes.includes('helped_10')) {
    newAchievements.push('helped_10');
  }
  if (heat.rows[0]?.total_helped >= 50 && !existingTypes.includes('helped_50')) {
    newAchievements.push('helped_50');
  }
  if (heat.rows[0]?.is_golden_earbud && !existingTypes.includes('golden_earbud')) {
    newAchievements.push('golden_earbud');
  }

  for (const achievement of newAchievements) {
    await pool.query(
      'INSERT INTO achievements (anonymous_id, achievement_type) VALUES ($1, $2)',
      [anonymous_id, achievement]
    );
    await pool.query(
      'INSERT INTO notifications (anonymous_id, message) VALUES ($1, $2)',
      [anonymous_id, `🏆 Achievement unlocked: ${achievement.replace('_', ' ').toUpperCase()}!`]
    );
  }
...
app.get('/api/balance', async (req, res) => {
  ...
  const notifications = await pool.query(
    'SELECT id, message, is_read, created_at FROM notifications WHERE anonymous_id = $1 AND is_read = false ORDER BY created_at DESC LIMIT 5',
    [anonymous_id]
  );
...
app.post('/api/call/end', async (req, res) => {
  ...
  await pool.query(
    'UPDATE sessions SET ended_at = NOW(), ended_early = $1, call_duration = $2 WHERE session_id = $3 AND seeker_id = $4',
    [ended_early || false, call_duration || 0, session_id, anonymous_id]
  );
  ...
  if (helper.rows.length > 0) {
    ...
    if (!ended_early) {
      await pool.query(
        'UPDATE heat SET total_helped = total_helped + 1 WHERE anonymous_id = $1',
        [helperId]
      );
    }
    ...
    if (bonus > 0) {
      ...
      await pool.query(
        'INSERT INTO notifications (anonymous_id, message) VALUES ($1, $2)',
        [anonymous_id, `🎉 ${newStreak}-day streak! You earned ${bonus} bonus minutes!`]
      );
    }
...
app.post('/api/rating', async (req, res) => {
  ...
  if (session.rows.length > 0) {
    ...
    if (ratings.rows[0].count >= 5 && ratings.rows[0].avg_rating >= 4.5) {
      ...
      await pool.query(
        'INSERT INTO notifications (anonymous_id, message) VALUES ($1, $2)',
        [helperId, '👑 Congratulations! You are now a Golden Earbud! You earn double minutes!']
      );
    }
  }
...
app.post('/api/mood/log', async (req, res) => {
  ...
  await pool.query(
    'UPDATE users SET last_active = NOW() WHERE anonymous_id = $1',
    [anonymous_id]
  );
...
app.get('/api/leaderboard', async (req, res) => {
  const leaderboard = await pool.query(
    'SELECT anonymous_id, score, streak, is_golden_earbud, total_helped FROM heat ORDER BY score DESC LIMIT 10'
  );
...
app.get('/api/notifications', async (req, res) => {
  ...
  const notifications = await pool.query(
    'SELECT id, message, is_read, created_at FROM notifications WHERE anonymous_id = $1 ORDER BY created_at DESC LIMIT 20',
    [anonymous_id]
  );
...
app.post('/api/notifications/read', async (req, res) => {
  ...
  await pool.query(
    'UPDATE notifications SET is_read = true WHERE id = $1 AND anonymous_id = $2',
    [notification_id, anonymous_id]
  );
